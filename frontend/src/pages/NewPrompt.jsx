...
function NewPrompt() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post("api/auth/prompts", promptData, {
        headers: {
    ...
    return (
      <div className="text-center mb-6">
        ...
        <div className="mt-2 text-gray-400 text-sm max-w-md mx-auto">
          {step === 1 &&
            "Choose a descriptive title that clearly explains what your prompt does."}
          {step === 2 &&
            "Enter the exact prompt text that users should copy into their AI tool."}
          {step === 3 &&
            "Add an example of what the AI typically outputs with this prompt."}
          {step === 4 &&
            "Select AI model and add tags to help others find your prompt."}
          {step === 5 &&
            "Review all details before submitting your prompt to the community."}
        </div>
      </div>
    );
  };
        ...
        return (
          <div className="animate-fade-in">
            ...
            <div className="mt-4">
              <div className="flex flex-wrap gap-2 mt-2">
                {tags.map((tag, index) => (
                  <div
                    key={index}
                    className="inline-flex items-center px-3 py-1.5 rounded-full bg-blue-900/50 text-blue-300 text-sm border border-blue-800/50"
                  >
                    <span>{tag}</span>
                    <button
                      type="button"
            ...
            <div className="mt-6">
              ...
              {!imagePreview ? (
                <div
                  className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-700 border-dashed rounded-lg hover:border-gray-500 transition-colors cursor-pointer bg-gray-800/50"
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {tags.length > 0 ? (
                        tags.map((tag, index) => (
                          <span
                            key={index}
                            className="inline-flex items-center px-2 py-1 rounded-full bg-blue-900/30 text-blue-300 text-xs"
                          >
                            {tag}
                          </span>
                        ))
    ...
    <div className="min-h-screen bg-gray-950 py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        <p className="text-center text-gray-400 mb-8">
          Help others unleash the full potential of AI models with your creative
          prompts
        </p>
